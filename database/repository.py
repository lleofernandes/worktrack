...
Isola todo acesso ao banco de dados. Os services NÃO fazem queries diretas —
apenas chamam métodos deste módulo.
...
class ContractRateRepository:
    ...
    def get_active_rate(
        ...
        """
        Retorna o rate vigente para empresa na data informada.
    ...
    def close_current_rate(
        session: Session, company_id: int, end_date: date
    ) -> Optional[ContractRateHistory]:
        """Fecha o rate vigente (end_date = None) definindo uma data de término."""
        current = (
            session.query(ContractRateHistory)
            .filter(
                ContractRateHistory.company_id == company_id,
...
class WorkLogRepository:
    ...
    def list_filtered(
        ...
        q = session.query(WorkLog).order_by(
            WorkLog.date.desc(), WorkLog.start_time.desc()
        )
        if company_id:
...
class HolidayRepository:
    ...
    def create(
        ...
        holiday = Holiday(date=date_, description=description, is_national=is_national)
        session.add(holiday)
        session.flush()
        return holiday
