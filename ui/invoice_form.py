...
def _reset_new_invoice_keys() -> None:
    keys_to_clear = [
        "inv_contract",
        "inv_issue_date", "inv_invoice_number",
        "inv_amount", "inv_origin", "inv_notes",
    ]
...
def _render_new(session) -> None:
    ...
    # clear_on_submit=True já limpa campos internos do st.form após submit bem-sucedido
    with st.form("form_new_invoice", clear_on_submit=True):
        col1, col2 = st.columns(2)
        with col1:
            issue_date     = st.date_input("Data de Emissão *", value=date.today(), format="DD/MM/YYYY", key="inv_issue_date")
        with col2:
            invoice_number = st.text_input("Número da NF *", key="inv_invoice_number")

        col3, col4 = st.columns(2)
        with col3:
            amount = st.number_input("Valor (R$) *", min_value=0.0, step=100.0, format="%.2f", key="inv_amount")
        with col4:
            origin = st.text_input("Origem / Referência", placeholder="Ex: Competência Mar/2025", key="inv_origin")

        notes     = st.text_input("Observações", max_chars=255, value="Serviços prestados de Analista de Dados", key="inv_notes")
